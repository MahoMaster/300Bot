...
func GetUserStory(name string, storyStr string, qq string, msg map[string]interface{}) {
	gi.goroutineRun(func() {
		...
		//模板
		template := name + `是一名修仙者，背景故事关键词为:` + storyStr + `（五灵根<四灵根<三灵根<双灵根<单灵根，灵根越少越好并且越稀有），请在70字内随机编写出` + name + `的身世故事，描写到准备踏入修仙之前即可.尽可能描述出关键词相关且请回避父母双亡等剧情，同时请不要出现平凡人却被人看重资质等矛盾剧情`
