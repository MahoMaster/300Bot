...
const chatJSONProtocolPrompt = "你必须只输出 JSON，不要输出任何其他文字。格式：{\"should_reply\":true,\"reply\":\"你的回复内容\",\"memory\":[\"记忆候选\"]}。" +
	...
	"（每条一句完整陈述句，涉及人物必须引用QQ号）。" +
	"memory 只记稳定的长期信息：身份/昵称别名、长期居住地/职业、明确的喜好与厌恶、稳定的性格习惯、群成员关系、群内梗、用户要求记住的内容；" +
	"禁止记录一次性事件（问了什么问题、今天玩了什么、单次情绪、天气/搜索咨询）。" +
	"没有可提取的内容就给空数组，宁缺毋滥。"
